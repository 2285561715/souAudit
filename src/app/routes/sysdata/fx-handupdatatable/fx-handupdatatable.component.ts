...
export class SysdataFxHandupdatatableComponent implements OnInit {
  ...
  editInfo(record: any[]) {
    this.modal.create(SysdataFxHandupdatatableEditComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }

  viewInfo(record: any[]) {
    this.modal.create(SysdataFxHandupdatatableViewComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }
