...
export class SysdataZxHandupdatatableComponent implements OnInit {
  ...
  editInfo(record: any[]) {
    this.modal.create(SysdataZxHandupdatatableEditComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }

  viewInfo(record: any[]) {
    this.modal.create(SysdataZxHandupdatatableViewComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }
