...
export class SysdataFxSynchdatatableComponent implements OnInit {
  ...
  editInfo(record: any[]) {
    this.modal.create(SysdataFxSynchdatatableEditComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }

  viewInfo(record: any[]) {
    this.modal.create(SysdataFxSynchdatatableViewComponent, { record }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }
