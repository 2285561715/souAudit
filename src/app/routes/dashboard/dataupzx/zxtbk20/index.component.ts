...
export class DashboardDataUpZxtbK20IndexComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.upUrl =
      '/api/excel/import?tableName=sjzxtb_k20_kcjs&appId=' +
      this.dataStr.id +
      '&stepId=' +
      this.dataStr.stepId +
      '&deptId=' +
  ...
  loadInfo(): void {
    this.listOfData = [];
    // 获得数据表的数据
    this.http.get('/api/data/tables/search/zxtb/sjzxtb_k20_kcjs').subscribe((res: any[]) => {
      res.forEach(item => {
        item.id = item.id + '';
