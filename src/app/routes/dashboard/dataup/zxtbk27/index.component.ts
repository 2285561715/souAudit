...
@Component({
  selector: 'app-dashboard-dataup-zxtbk27-index',
  templateUrl: './index.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DashboardDataUpZxtbK27IndexComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    this.upUrl =
      '/api/excel/import?tableName=sjzxtb_k27_kfjygjyxl&appId=' +
      this.dataStr.id +
      '&stepId=' +
      this.dataStr.stepId +
      '&deptId=' +
    ...
    this.http
      .put(
        `/api/data/tables/entry?id=` +
          id +
          `&tableno=sjzxtb_k27_kfjygjyxl&appId=` +
          this.dataStr.id +
          `&stepId=` +
          this.dataStr.stepId +
          `&deptId=` +
  ...
  addData(): void {
    ...
    this.http
      .put(
        `/api/data/tables/entry/init?tableno=sjzxtb_k27_kfjygjyxl&nd=` +
          date.getFullYear() +
          `&appId=` +
          this.dataStr.id +
          `&stepId=` +
