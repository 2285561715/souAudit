...
@Component({
  selector: 'app-dashboard-dataup-zxtbk23-index',
  templateUrl: './index.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DashboardDataUpZxtbK23IndexComponent implements OnInit {
  constructor(
  ...
  loadInfo(): void {
    this.listOfData = [];
    // 获得数据表的数据
    this.http.get('/api/data/tables/search/zxtb/sjzxtb_k23_rcpy3l').subscribe((res: any[]) => {
      res.forEach(item => {
        item.id = item.id + '';
    ...
    this.http
      .put(
        `/api/data/tables/entry?id=` +
          id +
          `&tableno=sjzxtb_k23_rcpy3l&appId=17&stepId=21&deptId=` +
