...
export class DashboardDataUpZxtbK26IndexComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.upUrl = '/api/excel/import?tableName=sjzxtb_k26_sbjhykyj&appId=17&stepId=21&deptId=' + this.loadUser.user.bid;
    this.loadInfo();
  }
  // 获得数据表的数据
  loadInfo(): void {
    this.listOfData = [];
    this.http.get('/api/data/tables/search/sjzxtb_k26_sbjhykyj').subscribe((res: any[]) => {
      res.forEach(item => {
        item.id = item.id + '';
    ...
    this.http
      .put(
        `/api/data/tables/entry?id=` +
          id +
          `&tableno=sjzxtb_k26_sbjhykyj&appId=17&stepId=21&deptId=` +
