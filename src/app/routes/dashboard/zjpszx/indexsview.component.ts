...
import { DashboardZjpsZxInxCommentComponent } from './inxComment.component';
import { DashboardZjpsZxReviewComponent } from './review.component';
// import { DashboardDataUpZxSjtbComponent } from './zxsjtb.component';

@Component({
  selector: 'app-dashboard-zjpszx-indexs-view',
  templateUrl: './indexsview.component.html',
  styleUrls: ['./indexsview.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DashboardZjpsZxIndexsViewComponent implements OnInit {
  ...
  loadInfo(): void {
    ...
    this.parmOfSqlPoint = {
      ...
      fieldList: ['id', 'index_id', 'zj_id', 'ps_point', 'ps_text', 'up_time', 'mod_time', 'is_done'],
    ...
    zip(
      ...
      .pipe(
        catchError(([cxIndexs, cxFiles, cxPoints]) => {
          return [cxIndexs, cxFiles, cxPoints];
        }),
      )
      .subscribe(
        ([cxIndexs, cxFiles, cxPoints]) => {
        ...
        () => {
          ...
          // console.log(this.listOfDataPoint);
  ...
  openEdit(record: any) {
    ...
    this.modal.create(DashboardZjpsZxReviewComponent, { dataValue }, { size: 'xl' }).subscribe((res: any) => {
      this.loadInfo();
    });
  }
  // 指标内涵说明
  openInxcoments(record: any) {
    this.modal.create(DashboardZjpsZxInxCommentComponent, { record }, { size: 'lg' }).subscribe((res: any) => {});
  }
  // --------------------------------------------------------------
