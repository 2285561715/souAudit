...
export class DashboardDataUpFxtbB22IndexComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.upUrl =
      '/api/excel/import?tableName=sjzxtb_k22_hjqkxs&appId=' +
      this.dataStr.id +
      '&stepId=' +
      this.dataStr.stepId +
      '&deptId=' +
    ...
    this.http
      .put(
        `/api/data/tables/entry?id=` +
          id +
          `&tableno=sjzxtb_k22_hjqkxs&appId=` +
          this.dataStr.id +
          `&stepId=` +
          this.dataStr.stepId +
          `&deptId=` +
  ...
  deleteInfo() {
    this.http
      .delete(
        '/api/data/tables/entry/del/nd?tableNo=sjzxtb_k22_hjqkxs&tableLx=fx&deptId=' +
          this.loadUser.user.bid +
          '&nd=2016',
      )
      .subscribe((res: any) => {});
