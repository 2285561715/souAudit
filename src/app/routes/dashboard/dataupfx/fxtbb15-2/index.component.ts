...
@Component({
  selector: 'app-dashboard-dataup-fxtbb152-index',
  templateUrl: './index.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DashboardDataUpFxtbB152IndexComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    this.upUrl = '/api/excel/import?tableName=sjzxtb_k15_web&appId=18&stepId=29&deptId=' + this.loadUser.user.bid;
    this.loadInfo();
  }
  loadInfo(): void {
    this.listOfData = [];
    // 获得数据表的数据
    this.http.get('/api/data/tables/search/fxtb/sjzxtb_k15_web').subscribe((res: any[]) => {
      res.forEach(item => {
    ...
    this.http
      .put(
        `/api/data/tables/entry?id=` +
          id +
          `&tableno=sjzxtb_k15_web&appId=18&stepId=29&deptId=` +
